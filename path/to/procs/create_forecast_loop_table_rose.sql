create procedure "Decisioning_Procs"."Create_Forecast_Loop_Table_Rose"( 
...
begin
  ...
  if "Input_Table_Name" <> 'Forecast_Loop_Table' then
    ...
    execute immediate ''
      ...
      -- || ' ,Cast(null as float)  as rand_WC_Save_Vol '
       ...
       || ' ,Cast(null as float)  as rand_PC_model '
       || ' ,Cast(null as varchar(20))  as OC_model_out '
       || ' ,Cast(null as float)  as P_target000 '
       || ' ,Cast(null as float)  as P_target010 '
       || ' ,Cast(null as float)  as P_target100 '
       || ' ,Cast(null as float)  as P_target110 '
       || ' ,Cast(null as float)  as P_target001 '
       || ' ,Cast(null as float)  as P_target011 '
       || ' ,Cast(null as float)  as P_target101 '
       || ' ,Cast(null as float)  as P_target111 '
       || ' ,Cast(null as float)  as Rose_migration_in_next_7d '
      /*------------------------------------------------*/
      /* Rates -----------------------------------------*/
      /*------------------------------------------------*/
      -- BBCoE Call Rates
      ...
      -- Product Holding Movements
       || ' ,Cast(null as varchar(10)) as DTV_Status_Code_EoW, cast(null as varchar(100)) as DTV_Product_Holding_EoW '
  ...
  update "Forecast_Loop_Table"
    ...
    "rand_PC_model" = "rand"("number"()*"multiplier"+5895),
    "rand_OC_model" = "rand"("number"()*"multiplier"+2424);
  update "Forecast_Loop_Table"
    set "rand_Value_Call" = "rand_action_Cuscan"
    where "rand_PC_model" < .33;
  if "Input_Table_Name" = 'CITeam.Cust_Weekly_Base' then
  ...
  -- Call Create_Fcast_AB_Hist('Forecast_Loop_Table');
  end if
end
